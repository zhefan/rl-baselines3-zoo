...
from stable_baselines3 import A2C, DDPG, DQN, PPO, SAC, TD3, RepLearn, AutoEncoder
...
from various_env import RecordEnv
...
def get_latest_run_id(log_path: str, env_id: str) -> int:
    ...
    for path in glob.glob(os.path.join(log_path, env_id + "*_[0-9]*")):
        ...
        if env_id == file_name.split("_")[0] and ext.isdigit() and int(ext) > max_run_id:
            max_run_id = int(ext)
    return max_run_id
